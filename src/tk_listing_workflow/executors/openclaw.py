...
class OpenClawExecutor:
    ...
    def _build_main_prompt(self, prompt_mode: str, payload: dict[str, Any], variables: dict[str, Any]) -> str:
        ...
        base = [
            f"{product_name}，1:1 电商主图，真实商业摄影质感，主体清晰完整，画面自然高级，点击感强。",
            f"视觉重点：{hero_focus}；风格：{style_summary}；场景氛围：{usage}。",
            f"画面方向：{self._main_visual_direction(payload, prompt_mode)}。",
            "画面可优化背景、构图、光影、道具和空间层次，允许更高级的布景与镜头表达，但产品必须始终是视觉中心。",
            "产品锁定：外形、颜色、材质、包装、品牌识别和结构细节与白底参考图一致，不改款、不变形、不新增无关配件。",
        ]
